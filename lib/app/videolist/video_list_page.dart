...
class FastPageScrollPhysics extends ScrollPhysics {
  ...
  static const double _flingThreshold = 0.3;

  // 느린 드래그에서 페이지를 넘기는 데 필요한 이동 비율(뷰포트 대비).
  // 0.5(50%, round)가 기본이지만, 0.03(3%)로 낮춰 화면 3%만 끌어도 전환 → 극도로 민감.
  static const double _commitFraction = 0.01;
...
class _VideoListPageState extends State<VideoListPage> with AutomaticKeepAliveClientMixin<VideoListPage> {
  ...
  static const int _adEveryNVideos = 10;
  static const int _blockSize = _adEveryNVideos + 1; // 영상10 + 광고1 = 11페이지가 한 블록
  ...
  Widget buildLocalName() {
    ...
    return Positioned(
        // 위 필터 칩 줄(top+10, 높이 약 30)과 붙지 않을 정도의 틈(약 10px)만 둔다.
        top: MediaQuery.of(context).padding.top + (Platform.isAndroid ? 50 : 40),
        left: 6,
        child: Obx(
          () {
