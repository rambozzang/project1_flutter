...
import 'package:project1/app/videolist/Video_screen_page2.dart';
...
class VideoListPage2 extends StatefulWidget {
  const VideoListPage2({super.key});

  @override
  State<VideoListPage2> createState() => _VideoListPage2State();
}

class _VideoListPage2State extends State<VideoListPage2> with AutomaticKeepAliveClientMixin<VideoListPage2> {
  @override
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        onRefresh: () async {
          Get.find<VideoListCntr2>().pageNum = 0;
          Get.find<VideoListCntr2>().getData();
        },
        child: Stack(
          children: [
            Positioned.fill(
              child: Utils.commonStreamList<BoardWeatherListData>(
                  Get.find<VideoListCntr2>().videoListCntr, buildVideoBody, Get.find<VideoListCntr2>().getData),
            ),
            buildLocalName(),
            // 상단 현재 온도
            buildTemp(),
  ...
  Widget buildVideoBody(List<BoardWeatherListData> data) {
    return PreloadPageView.builder(
        ...
        onPageChanged: (int position) {
          ...
          lo.g('@@@ onPageChanged position : $position ,  ${Get.find<VideoListCntr2>().list[position].boardId}');
          if (Get.find<VideoListCntr2>().currentIndex.value > position) {
            Get.find<VideoListCntr2>().playPrevious(position);
          } else {
            Get.find<VideoListCntr2>().playNext(position);
          }

          Get.find<VideoListCntr2>().currentIndex.value = position;
          Get.find<VideoListCntr2>().getMoreData(position, data.length);
        },
        itemBuilder: (context, i) {
          ...
          // return VideoScreenPage(index: i, data: data[i]);
          if (Get.find<VideoListCntr2>().videoPlayerControllerList[i] == null) {
            return Utils.progressbar(color: Colors.blue);
          }
          return VideoScreenPage2(index: i, controller: Get.find<VideoListCntr2>().videoPlayerControllerList[i], data: data[i]);
        });
  }
    ...
    return Positioned(
      ...
      child: Container(
        ...
        child: SingleChildScrollView(
          ...
          child: Obx(() => Column(
                children: Get.find<VideoListCntr2>().list.map((element) {
                  return Padding(
                      padding: const EdgeInsets.symmetric(horizontal: 15.0, vertical: 2.0),
                      child: Container(
                        decoration: BoxDecoration(
                          border: Border.all(
                              color: (Get.find<VideoListCntr2>().list[Get.find<VideoListCntr2>().currentIndex.value].boardId ==
                                      element.boardId)
                                  ? Colors.yellow
                                  : Colors.transparent,
                              width: 1),
                        ...
                        child: Text(element.boardId.toString(),
                            style: TextStyle(
                              color: (Get.find<VideoListCntr2>().mountedList.contains(element.boardId)) ? Colors.white : Colors.yellow,
                              fontSize: 12,
                            )),
                      ));
                }).toList(),
