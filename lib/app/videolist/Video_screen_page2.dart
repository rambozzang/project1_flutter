...
class VideoScreenPage2 extends StatefulWidget {
  const VideoScreenPage2({super.key, required this.index, required this.data, required this.controller});
...
class VideoScreenPage2State extends State<VideoScreenPage2> {
  ...
  Future<void> initiliazeVideo() async {
    lo.g('@@@  VideoScreenPage2State initiliazeVideo() index : ${widget.index} : $initialized');
    ...
    try {
      // String finalUrl = widget.data.videoPath.toString().replaceAll('m3u8', 'mpd');
      String finalUrl = widget.data.videoPath.toString();
      lo.g('@@@  VideoScreenPage2State initiliazeVideo() Loading : ${widget.data.boardId}');

      //  File sfile = await DefaultCacheManager().getSingleFile(widget.data.videoPath.toString(), key: widget.data.boardId.toString());
      // _controller = VideoPlayerController.file(sfile);
      //  lo.g('@@@  sfile : ${sfile.lengthSync() / 1000}kb');
      // lo.g('@@@  sfile : ${sfile.lengthSync() / 1000 / 1000}Mb');

      // await Future.delayed(const Duration(milliseconds: 10000));

      // _controller = VideoPlayerController.networkUrl(Uri.directory(sfile.path),
      ...
      //       initialized = true;
      //     });
      //     lo.g('@@@  VideoScreenPage2State initiliazeVideo() Mounted : ${widget.data.boardId}');
      //     Get.find<VideoListCntr2>().onPageMounted(widget.data.boardId!);
      //   }
      // });
  ...
  Future<void> like() async {
    try {
      ...
      int inx = Get.find<VideoListCntr2>().currentIndex.value;
      // 현재 리스트에 좋아요 카운트 증가
      Get.find<VideoListCntr2>().list[inx].likeCnt = (Get.find<VideoListCntr2>().list[inx].likeCnt! + 1);
      // 현재 리스트에 좋아요 여부 변경
      Get.find<VideoListCntr2>().list[inx].likeYn = 'Y';
    } catch (e) {
  ...
  Future<void> likeCancle() async {
    try {
      ...
      Get.find<VideoListCntr2>().list[inx].likeCnt = (Get.find<VideoListCntr2>().list[inx].likeCnt! - 1);
      // 현재 리스트에 좋아요 여부 변경
      Get.find<VideoListCntr2>().list[inx].likeYn = 'N';
    } catch (e) {
      Utils.alert('좋아요 실패! 다시 시도해주세요');
    }
  }
                ...
                onVisibilityChanged: (info) {
                  ...
                  if (info.visibleFraction > 0.2) {
                    if (initialized) {
                      ...
                      Get.find<VideoListCntr2>().soundOff.value ? _controller.setVolume(0) : _controller.setVolume(1);
                    }
                        ...
                        return ElevatedButton(
                          onPressed: () {
                            if (widget.data.followYn.toString() == 'N') {
                              ...
                              Get.find<VideoListCntr2>().follow(widget.data.custId.toString());
                            } else {
                              widget.data.followYn = 'N';
                              Get.find<VideoListCntr2>().followCancle(widget.data.custId.toString());
                            }
                            setState(() {});
                          },
  ...
  Widget buildSoundButton() {
    return Positioned(
      ...
      child: Obx(() => IconButton(
            onPressed: () {
              Get.find<VideoListCntr2>().soundOff.value = !Get.find<VideoListCntr2>().soundOff.value;
              if (Get.find<VideoListCntr2>().soundOff.value) {
                _controller.setVolume(0);
              } else {
                _controller.setVolume(1);
              }
            },
            icon: Get.find<VideoListCntr2>().soundOff.value
