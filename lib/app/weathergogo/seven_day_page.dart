...
class _SevenDayPageState extends State<SevenDayPage> {
      ...
      child: GetBuilder<WeatherGogoCntr>(
        builder: (cntr) {
          ...
          return ListView.builder(
            ...
            itemCount: cntr.sevenDayWeather.length,
            itemBuilder: (context, index) => _buildListItem(cntr.sevenDayWeather[index], index, context),
          );
        },
      ),
    );
  ...
  Widget _buildListItem(SevenDayWeather weather, int index, BuildContext context) {
    return Material(
      ...
      child: InkWell(
        borderRadius: BorderRadius.circular(12.0),
        onTap: () => Get.toNamed('/SevendayDetailPage/'),
        child: Container(
  ...
  Widget _buildTemperatureGauge(SevenDayWeather weather, BuildContext context) {
    String minTemp = weather.morning.minTemp!.split('.')[0];
    String maxTemp = weather.afternoon.maxTemp!.split('.')[0];

    return Row(
      children: [
        Text('$minTemp°', style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: Colors.white)),
        const Gap(5),
        Obx(() => SizedBox(
        ...
        const Gap(5),
        Text('$maxTemp°', style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: Colors.white)),
      ],
    );
  }
}
