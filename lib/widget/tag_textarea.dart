...
class SuggestedDataWrapper<T> {
  ...
  bool operator ==(Object other) {
    return other is SuggestedDataWrapper && other.prefix == this.prefix && other.id == this.id;
  }
...
class TypeAheadTextFieldController extends TextEditingController {
  ...
  TypeAheadTextFieldController(
      ...
      this.onRemove,
      this.edgePadding = const EdgeInsets.only(left: 8, right: 0, top: 20, bottom: 0),
      this.onStateChanged,
  ...
  TextSpan buildTextSpan({BuildContext? context, TextStyle? style, bool? withComposing}) {
    ...
    if (_suggestibleData != null && _approvedData.length > 0) {
      ...
      _prefixes!.forEach((e) {
        var matchedWithPrefix = _approvedData.where((element) => element.prefix == e).map((e) => e.id).toSet();
        if (matchedWithPrefix.length > 0) {
          patterns.add(r'(?<=' + e + ')(' + '${matchedWithPrefix.map((e) => '$e').join('|')}' + r')');
        }
      });
    ...
    if (_lastCheckedText == null || _lastCheckedText != text) {
      try {
        ...
        Rect caretPrototype = Rect.fromLTWH(0.0, 0.0, _cursorWidth, _cursorHeight);
        ...
        Offset positiveOffset = Offset(
          ...
          (caretOffset.dy > 0 ? caretOffset.dy : -caretOffset.dy) + preferredLineHeight,
        );
  ...
  void approveSelection(PrefixMatchState state, SuggestedDataWrapper data) {
    ...
    String newText = this.text.replaceRange(position - replacedLength, position, data.id + ' ');
  ...
  void _selectionCheck(Offset dimensionalOffset, double cursorHeight) {
    if (selection.start > 0) {
      ...
      if (texts.length > 0) {
        ...
        var str = appliedPrefixes.map((prefix) => r'(' + prefix + '.*)').join('|');
        ...
        if (generalRegexp.hasMatch(texts.last)) {
          ...
          if (scrollController.hasClients) {
            if (scrollController.position.axis == Axis.horizontal) {
              _scrollOffset = this.scrollController.position.pixels / devicePixelRatio!;
            } else {
          ...
          Offset physicalOffset = new Offset(physicalOffsetDx + (edgePadding?.left ?? 0), physicalOffsetDy);

          WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
            onStateChanged!(PrefixMatchState(prefix, texts.last, offset: physicalOffset));
          });
        } else {
          WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
            onStateChanged!(null);
  ...
  Offset? calculateGlobalOffset(BuildContext context, Offset? localOffset, {double? dialogHeight, double? dialogWidth}) {
    if (localOffset != null) {
      RenderBox? tfBox = textFieldKey.currentContext?.findRenderObject() as RenderBox?;

      double? screenWidth;
      double? screenHeight;
      if (tfBox != null) {
        screenWidth = MediaQuery.of(context).size.width;
        screenHeight = MediaQuery.of(context).size.height - MediaQuery.of(context).viewInsets.bottom;
      }
      ...
      if (globalOffset != null) {
        ...
        if (dialogHeight != null && (dy + dialogHeight) > screenHeight!) {
          topPadding = (dy - dialogHeight - _cursorHeight - (edgePadding?.top ?? 0.0));
          if (topPadding > (screenHeight - dialogHeight)) {
            topPadding = screenHeight - dialogHeight - _cursorHeight - (edgePadding?.top ?? 0.0);
          }
        } else {
