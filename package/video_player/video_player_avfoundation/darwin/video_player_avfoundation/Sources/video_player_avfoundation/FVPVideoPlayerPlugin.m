...
@property(nonatomic) float networkBufferLevel; // 네트워크 상태에 따른 버퍼 레벨 (0.0 ~ 1.0)
@property(nonatomic) NSTimeInterval minBufferDuration; // 최소 버퍼 시간
@property(nonatomic) NSTimeInterval maxBufferDuration; // 최대 버퍼 시간
@property(nonatomic) NSTimeInterval currentBufferDuration; // 현재 버퍼 시간

@property(nonatomic) float preferredPeakBitRate;  // 선호 최대 비트레이트
@property(nonatomic) float currentBitRate;        // 현재 비트레이트
@property(nonatomic) float minBitRate;            // 최소 비트레이트
@property(nonatomic) float maxBitRate;            // 최대 비트레이트

// 리소스 관리를 위한 프로퍼티 추가
@property(nonatomic) NSOperationQueue *resourceManagementQueue;
@property(nonatomic) BOOL isBackgrounded;
@property(nonatomic) NSTimer *resourceMonitorTimer;
...
- (instancetype)initWithPlayerItem:(AVPlayerItem *)item
  ...
  // 비트레이트 초기 설정
  _minBitRate = 300000;    // 300Kbps
  _maxBitRate = 5000000;   // 5Mbps
  _preferredPeakBitRate = _maxBitRate;
  _currentBitRate = _maxBitRate;
  
  // 플레이어 비트레이트 설정
  [self updateBitRate];

  // 리소스 관리 큐 초기화
  _resourceManagementQueue = [[NSOperationQueue alloc] init];
  _resourceManagementQueue.maxConcurrentOperationCount = 1;
  ...
  if (context == timeRangeContext) {
    ...
    if ([[_player currentItem] isPlaybackLikelyToKeepUp]) {
      if (_eventSink != nil) {
        _eventSink(@{
          @"event": @"bufferingEnd",
          @"bufferDuration": @(self.currentBufferDuration),
          @"networkQuality": @(self.networkBufferLevel)
        });
      }
    } else {
      if (_eventSink != nil) {
...
- (void)dispose {
    ...
    [self.resourceManagementQueue cancelAllOperations];
    self.resourceManagementQueue = nil;
...
- (void)startNetworkMonitoring {
    ...
    [NSTimer scheduledTimerWithTimeInterval:1.0
        ...
        [self adjustBufferDuration];
    }];
}

- (void)adjustBufferDuration {
    AVPlayerItem *playerItem = self.player.currentItem;
    if (!playerItem) return;
    
    // 현재 버퍼된 시간 범위 계산
    CMTimeRange timeRange = [[playerItem loadedTimeRanges].firstObject CMTimeRangeValue];
    Float64 bufferedDuration = CMTimeGetSeconds(timeRange.duration);
    
    // 버퍼 상태에 따른 네트워크 상태 평가
    if (bufferedDuration < self.currentBufferDuration * 0.3) {
        // 버퍼가 낮은 상태 - 네트워크가 좋지 않음
        self.networkBufferLevel = MAX(0.0, self.networkBufferLevel - 0.2);
        // 비트레이트 낮추기
        [self decreaseBitRate];
    } else if (bufferedDuration > self.currentBufferDuration * 0.8) {
        // 버퍼가 충분한 상태 - 네트워크가 좋음
        self.networkBufferLevel = MIN(1.0, self.networkBufferLevel + 0.1);
        // 비트레이트 높이기
        [self increaseBitRate];
    }
    
    // 새로운 버퍼 시간 계산 및 업데이트
    [self updateBufferConfiguration];
}

- (void)decreaseBitRate {
    float decreaseAmount = (self.maxBitRate - self.minBitRate) * 0.2;
    self.preferredPeakBitRate = MAX(self.minBitRate, self.preferredPeakBitRate - decreaseAmount);
    [self updateBitRate];
}

- (void)increaseBitRate {
    float increaseAmount = (self.maxBitRate - self.minBitRate) * 0.1;
    self.preferredPeakBitRate = MIN(self.maxBitRate, self.preferredPeakBitRate + increaseAmount);
    [self updateBitRate];
}
...
- (void)updateBitRate {
    ...
    playerItem.preferredPeakBitRate = self.preferredPeakBitRate;
    self.currentBitRate = self.preferredPeakBitRate;
    
    // 비트레이트 변경 이벤트 전송
    if (self.eventSink) {
        self.eventSink(@{
            @"event": @"bitrateUpdate",
            @"bitrate": @(self.currentBitRate),
            @"networkQuality": @(self.networkBufferLevel)
        });
    }
}

- (void)updatePlayerBufferConfiguration {
    AVPlayerItem *playerItem = self.player.currentItem;
    if (!playerItem) return;
    
    // 새로운 버퍼 구성 설정
    playerItem.preferredForwardBufferDuration = self.currentBufferDuration;
...
- (void)startResourceMonitoring {
    self.resourceMonitorTimer = [NSTimer scheduledTimerWithTimeInterval:5.0
                                                               repeats:YES
                                                                 block:^(NSTimer * _Nonnull timer) {
        [self checkAndOptimizeResources];
    }];
}

- (void)checkAndOptimizeResources {
    [self.resourceManagementQueue addOperationWithBlock:^{
        ...
        // 메모리 사용량 최적화
        if (self.isBackgrounded || !self.isPlaying) {
            [self releaseUnusedResources];
        }
        
        // 버퍼 최적화
        [self optimizeBufferSize];
        
        // 비트레이트 최적화
        [self optimizeBitrate];
    }];
}

- (void)releaseUnusedResources {
    if (!self.isPlaying) {
        // 재생 중이 아닐 때 불필요한 리소스 해제
    ...
    if (self.isBackgrounded) {
        self.player.currentItem.preferredForwardBufferDuration = self.minBufferDuration;
        [self decreaseBitRate];
    }
}

- (void)optimizeBufferSize {
    Float64 currentMemoryUsage = [self getCurrentMemoryUsage];
    Float64 memoryLimit = [self getSystemMemoryLimit];
    
    if (currentMemoryUsage > memoryLimit * 0.7) {  // 메모리 사용량이 70% 이상일 때
        // 버퍼 크기 감소
        self.currentBufferDuration = self.minBufferDuration;
        [self updatePlayerBufferConfiguration];
    }
}

- (void)optimizeBitrate {
    // 시스템 리소스 상태에 따른 비트레이트 최적화
    Float64 currentMemoryUsage = [self getCurrentMemoryUsage];
    Float64 memoryLimit = [self getSystemMemoryLimit];
    
    if (currentMemoryUsage > memoryLimit * 0.8) {  // 메모리 사용량이 80% 이상일 때
        [self decreaseBitRate];
    }
}

- (Float64)getCurrentMemoryUsage {
    ...
    if (kernReturn == KERN_SUCCESS) {
        return (Float64)vmInfo.phys_footprint;
    }
    return 0;
}

- (Float64)getSystemMemoryLimit {
    return [[NSProcessInfo processInfo] physicalMemory] * 0.8;  // 시스템 메모리의 80%를 limit으로 설정
}
...
- (void)handleAppWillEnterForeground {
    ...
    [self.resourceManagementQueue addOperationWithBlock:^{
        // 포그라운드 진입 시 리소스 복구
        if (self.isPlaying) {
            [self restorePlaybackResources];
        }
    }];
}

- (void)restorePlaybackResources {
    // 포그라운드 진입 시 필요한 리소스 복구
    if (self.isPlaying) {
        self.currentBufferDuration = self.maxBufferDuration;
        [self updatePlayerBufferConfiguration];
        [self updateBitRate];
    }
}
...
- (void)updateBufferConfiguration {
    ...
    // 새로운 버퍼 시간 계산
    NSTimeInterval newBufferDuration = self.minBufferDuration + 
        (self.maxBufferDuration - self.minBufferDuration) * (1.0 - self.networkBufferLevel);
    
    // 버퍼 시간 업데이트
    if (ABS(newBufferDuration - self.currentBufferDuration) > 0.5) {
        self.currentBufferDuration = newBufferDuration;
        [self updatePlayerBufferConfiguration];
    }
}
...
- (void)detachFromEngineForRegistrar:(NSObject<FlutterPluginRegistrar> *)registrar {
  [self.playersByTextureId.allValues makeObjectsPerformSelector:@selector(dispose)];
